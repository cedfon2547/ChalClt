...
public class Rasterizer {
        ...
        // private Color getPixelColor(TriangleMesh object, Color color, Vector3D norm,
        // Vector3D pixelPoint) {
        ...
        // double ka = object.getMaterial().getAmbient() +
        // scene.getLight().getAmbientIntensity() + scene.getLight().getIntensity();
        ...
        // // Calculate the ambient component of the lighting model using the blended
        // color
        ...
        // // Calculate the diffuse component of the lighting model using the blended
        // color
        ...
        // // reflection model
        // // Vector3D viewDir =
        // scene.getCamera().getPosition().sub(pixelPoint).normalize();
        // // Vector3D reflectDir = norm.multiplyScalar(2 *
        // norm.dotProduct(lightDir)).sub(lightDir).normalize();
        // double specularFactor = 0; // Math.pow(Math.max(0,
        // viewDir.dotProduct(reflectDir)), n);
        // if (specularFactor < 0) {
        // specularFactor = 0;
        // }
        ...
        // double attenuation = 1 / (1 + 0.5 * lightDistance + 0.01 * lightDistance *
        // lightDistance);

        // // Modify the diffuse and specular calculations to include the attenuation
        // Color diffuseColor = new Color(
        // (int) Math.min(255, (kd * diffuseFactor * blendedColor.getRed() *
        // attenuation)),
        // (int) Math.min(255, (kd * diffuseFactor * blendedColor.getGreen() *
        // attenuation)),
        // (int) Math.min(255, kd * diffuseFactor * blendedColor.getBlue() *
        // attenuation),
        // (int) objectColor.getAlpha());
        ...
        // ambientColor.getGreen() + diffuseColor.getGreen() +
        ...
        // ambientColor.getAlpha() + diffuseColor.getAlpha() +
        ...
        private BufferedImage rasterize(Dimension panelDimension) {
                ...
                for (TriangleMesh obj : tMeshes) {
                        ...
                        if (obj.isSelected()) {
                                ...
                                g2.drawRect((int) bounds[0].getX() - 5, (int) bounds[0].getY() - 5,
                                                (int) obj.getWidth() + 10,
                                                (int) obj.getHeight() + 10);
                        }
                }
        ...
        public TriangleMesh getMeshFromPoint(Point point) {
                ...
                tMeshes.sort((TriangleMesh m1, TriangleMesh m2) -> {
                        double m1Z = 0;
                        double m2Z = 0;

                        for (Triangle t : m1.getTriangles()) {
                                m1Z += t.getCenter().z;
                        }

                        for (Triangle t : m2.getTriangles()) {
                                m2Z += t.getCenter().z;
                        }
                ...
                for (TriangleMesh tMesh : tMeshes) {
                        Vector3D[] bounding = tMesh.getBounding();

                        if (point.x >= bounding[0].x && point.x <= bounding[1].x && point.y >= bounding[0].y
                                        && point.y <= bounding[1].y) {
                                return scene.getMesh(tMesh.ID);
                        }
                }
