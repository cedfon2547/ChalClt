        ...
        for (TriangleMeshGroup group : scene.getMeshes()) {
            ...
            for (TriangleMesh obj : group.getMeshes()) {
                ...
                for (Triangle triangle : obj.getTriangles()) {
                    // this.rasterizeTriangle(g2, group, triangle);

                    Vector3D[] vertices = triangle.getVertices();

                    final Vector3D vertex1 = new Vector3D(vertices[0]).multiply(_transform);
                    final Vector3D vertex2 = new Vector3D(vertices[1]).multiply(_transform);
                    final Vector3D vertex3 = new Vector3D(vertices[2]).multiply(_transform);
